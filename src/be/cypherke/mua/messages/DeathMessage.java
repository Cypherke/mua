...
public class DeathMessage extends MessageBase {
    ...
    public boolean handle(String function, String message) throws IOException {
        String[] deathMessages = {"was slain by Zombie", "was blown up by Creeper", "was slain by Enderman", "tried to swim in lava",
            "hit the ground too hard", "was shot by Skeleton"};
        for (String deathMessage : deathMessages) {
            if (message.contains(deathMessage)) {
                ...
                getMua().getOutput().sendMessage("@a", "Congrats " + message.split(" ")[0]
                    + ", this brings your total death count to: " + getMua().getUsersDb().getUser(user).getNumberOfDeaths());
